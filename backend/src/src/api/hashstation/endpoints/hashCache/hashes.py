...
from src.api.hashstation.endpoints.hashCache.argumentsParser import hashes_parser, hashList_argument
from src.api.hashstation.endpoints.hashCache.responseModels import page_of_hashes_model
...
from src.database.models import HsHash
...
class hashCache(Resource):
    ...
    def get(self):
        ...
        hashes = HsHash.query.filter_by(deleted=False)


        if args.search:
            print('%' + args.search.encode().hex() + '%')
            hashes = hashes.filter(or_(HsHash.result.like(bytes('%' + args.search.encode().hex() + '%', "ascii")),
                                       HsHash.hash.like(bytes('%' + args.search + '%', "utf-8"))))
        ...
        if args.order_by:
            ...
            orderBy = getattr(HsHash, args.order_by)
            if args.descending:
                orderBy = orderBy.desc()
                hashes = hashes.order_by(orderBy)

        else:
            hashes = hashes.order_by(HsHash.id.desc())
    ...
    def patch(self):
        ...
        ids = args['hash_ids']
        query = HsHash.query.filter(HsHash.id.in_(ids))
...
class exportCrackedHashes(Resource):
    def get(self):
        ...
        hashes = hashes.filter(HsHash.result != None).all()
...
class importCrackedHashes(Resource):
    ...
    def post(self):
        ...
        for row in csv_reader:
            ...
            new_hash = HsHash(hash_type=hash_type, hash=hash_bytes, result=result, time_cracked=datetime.datetime.utcnow())
