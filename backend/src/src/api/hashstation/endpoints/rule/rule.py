...
from src.api.hashstation.endpoints.rule.argumentsParser import updateRule_parser, rule_parser
from src.api.hashstation.endpoints.rule.responseModels import rules_model, rule_model, ruleData_model
from src.api.hashstation.functions import fileUpload
from src.api.hashstation.responseModels import simpleResponse
from src.database import db
from src.database.models import HsRule
...
class ruleCollection(Resource):
            ...
            with open(os.path.join(RULE_DIR, uploadedFile['path']), encoding='latin-1') as file:
                for line in file:
                    if re.match('^\s*(\#.*)?$', line) == None:
                        rule_count += 1
            rule = HsRule(name=uploadedFile['filename'], path=uploadedFile['path'], count=rule_count)
            try:
                db.session.add(rule)
                db.session.commit()
    ...
    def get(self):
        ...
        """
        return {'items': HsRule.query.filter(HsRule.deleted == False).all()}
...
class rule(Resource):
    ...
    def get(self, id):
        ...
        rule = HsRule.query.filter(HsRule.id == id).first()
    ...
    def delete(self, id):
        ...
        Deletes rule file.
        """
        rule = HsRule.query.filter(HsRule.id == id).one()
        rule.deleted = True
        db.session.commit()
