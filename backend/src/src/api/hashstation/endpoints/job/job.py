...
from src.api.hashstation.endpoints.host.argumentsParser import jobHost_parser
from src.api.hashstation.endpoints.host.responseModels import page_of_hosts_model
from src.api.hashstation.endpoints.job.argumentsParser import jobList_parser, jobWorkunit_parser, \
...
from src.api.hashstation.endpoints.job.functions import verifyHashes, create_job, \
...
from src.api.hashstation.endpoints.job.responseModels import page_of_jobs_model, page_of_jobs_model, \
...
from src.api.hashstation.attacks.functions import compute_prince_keyspace
from src.api.hashstation.lang import status_to_code, job_status_text_to_code_dict, attack_modes
from src.api.hashstation.responseModels import simpleResponse
from src.database import db
from src.database.models import HsJob, Host, HsHost, HsWorkunit, HsHostActivity, HsMask, HsJobGraph, \
    HsJobDictionary, HsPcfg, HsJobStatus, HsRule, HsUserPermission, HsUser
...
class jobsCollection(Resource):
    ...
    def get(self):
        ...
        jobs_query = HsJob.query

        if not current_user.role.VIEW_ALL_JOBS:
            ids = visible_jobs_ids()
            jobs_query = jobs_query.filter(HsJob.id.in_(ids))

        if args.bin is not None:
            jobs_query = jobs_query.filter(HsJob.bins.any(id=args.bin))
        ...
        if args.name:
            jobs_query = jobs_query.filter(HsJob.name.like('%' + args.name + '%'))
        ...
        if args.order_by:
            orderBy = getattr(HsJob, args.order_by)
            if args.descending:
                orderBy = orderBy.desc()
            jobs_query = jobs_query.order_by(orderBy)

        else:
            jobs_query = jobs_query.order_by(HsJob.id.desc())
    ...
    def patch(self):
        ...
        ids = args['job_ids']
        query = HsJob.query.filter(HsJob.id.in_(ids))
        if not current_user.role.EDIT_ALL_JOBS:
            editable = editable_jobs_ids()
            query = query.filter(HsJob.id.in_(editable))
        jobs = query.all()
        if (len(jobs) == 0):
...
class multiJobsHost(Resource):
    ...
    def post(self):
        ...
        beforeHosts = HsHostActivity.query.filter(HsHostActivity.job_id.in_(ids)).all()
        ...
        for jobId in ids:
            for hostId in newIds:
                host = HsHostActivity(boinc_host_id=hostId, job_id=jobId)
                db.session.add(host)
...
class JobByID(Resource):
    ...
    def get(self, id):
        ...
        job = HsJob.query.filter(HsJob.id == id).one_or_none()
        if not job:
            abort(404, 'Job not found.')

        if job.grammar_id:
            pcfg = HsPcfg.query.filter(HsPcfg.id == job.grammar_id).one()
    ...
    def put(self, id):
        ...
        job = HsJob.query.filter(HsJob.id == id).one()
    ...
    def get(self, id):
        ...
        if action == 'start':
            ...
            if job.attack_mode == attack_modes['mask'] or job.attack_mode == attack_modes['hybrid (wordlist + mask)']:
                masks = HsMask.query.filter(HsMask.job_id == id).all()
                for mask in masks:
                ...
                dictionaries = HsJobDictionary.query.filter(HsJobDictionary.job_id == id).all()
                for dictionary in dictionaries:
                    dictionary.current_index = 0
            graphData = HsJobGraph.query.filter(HsJobGraph.job_id == id).all()
            for item in graphData:
                db.session.delete(item)
            db.session.add(HsJobGraph(progress=0, job_id=job.id))
        elif action == 'kill':
            kill_job(job, db)
        else:
...
class userPermissions(Resource):
    ...
    def get(self, id):
        ...
        """
        data = HsUserPermission.query.filter_by(job_id=id).all()
        return {
            'items': [{
                'id': d.user.id,
    ...
    def put(self, id):
        ...
        record = HsUserPermission.query.filter_by(job_id=id).filter_by(user_id=user_id).one_or_none()
        if not record:
            record = HsUserPermission(job_id=id, user_id=user_id, view=0, modify=0, operate=0, owner=0)
...
class jobsHost(Resource):
    ...
    def get(self, id):
        ...
        hosts_query = HsHost.query.filter_by(job_id=id)
    ...
    def post(self, id):
        ...
        beforeHosts = HsHostActivity.query.filter_by(job_id=id).all()
        for host in beforeHosts:
            db.session.delete(host)

        for hostId in args['newHost_ids']:
            host = HsHostActivity(boinc_host_id=hostId, job_id=id)
            db.session.add(host)
...
class workunitsJob(Resource):
    ...
    def get(self, id):
        ...
        jobs_query = HsWorkunit.query.filter_by(job_id=id)
...
class jobsInfo(Resource):
    ...
    def get(self):
        ...
        statusesCount = db.session.query(HsJob.status, func.count(HsJob.id)).group_by(HsJob.status).all()
        result = []
...
class lastJobs(Resource):
    ...
    def get(self):
        ...
        states = HsJobStatus.query.order_by(HsJobStatus.time.desc()).all()
        ...
        for jobId in ids:
            job = HsJob.query.filter(HsJob.id == jobId).one()
            if job.deleted == 0:
