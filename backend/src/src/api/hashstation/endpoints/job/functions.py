...
from src.api.hashstation.attacks import processJob as attacks
from src.api.hashstation.attacks.functions import compute_keyspace_from_mask, compute_prince_keyspace, compute_rules_keyspace
from src.api.hashstation.lang import status_to_code, attack_modes
from src.api.hashstation.functions import shellExec, lenStr
from src.database import db
from src.database.models import HsJob, HsHostActivity, HsBenchmark, Host, HsDictionary, HsJobDictionary, \
    HsJobGraph, HsRule, HsHash, HsMask, HsUserPermission, HsSettings, HsWorkunit, HsDeviceInfo
from src.api.hashstation.endpoints.pcfg.functions import extractNameFromZipfile
...
def kill_job(job, db):
    ...
    if (int(job.status) != status_to_code['running']) and (int(job.status) != status_to_code['finishing']):
        ...
        workunits = HsWorkunit.query.filter(HsWorkunit.job_id == id).all()
        for wu in workunits:
            device_info_for_wu = HsDeviceInfo.query.filter(HsDeviceInfo.workunit_id == wu.id).all()
    ...
    hosts = HsHostActivity.query.filter(HsHostActivity.job_id == id).all()
    for host in hosts:
        host.status = 0
...
def create_job(data):
    ...
    for host in data['hosts_ids']:
        ...
        db_host_activity = HsHostActivity(job_id=db_job.id, boinc_host_id=db_host.id)
        db.session.add(db_host_activity)

    for hashObj in data['hash_settings']['hash_list']:
        hash = HsHash(job_id=db_job.id, hash_type=job['hash_settings']['hash_type'], hash=hashObj['hash'], username=hashObj['username'])
    ...
    perms = HsUserPermission(user_id=current_user.id, job_id=db_job.id, view=1, modify=1, operate=1, owner=1)
...
def computeCrackingTime(data):
    ...
    if data['hash_type_code'] != -1 and len(data['boinc_host_ids']) > 0:
        hosts = HsBenchmark.query.filter(HsBenchmark.hash_type == data['hash_type_code']). \
            filter(HsBenchmark.attack_mode == attackSettings['attack_mode']). \
            filter(HsBenchmark.boinc_host_id.in_(data['boinc_host_ids'])).all()
...
# permission utils
def perm_base ():
    return db.session.query(HsUserPermission.job_id).filter_by(user_id=current_user.id)
