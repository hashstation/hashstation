...
from src.api.hashstation.endpoints.markov.argumentsParser import makeMarkovFromDictionary_parser
...
from src.api.hashstation.functions import fileUpload, shellExec
from src.api.hashstation.responseModels import simpleResponse
from src.database import db
from src.database.models import HsHcstat, HsDictionary
...
class markovCollection(Resource):
    ...
    def get(self):
        ...
        """
        return {'items': HsHcstat.query.filter(HsHcstat.deleted == False).all()}
...
class markovAdd(Resource):
    ...
    def post(self):
        ...
        if uploadedFile:
            hcstats = HsHcstat(name=uploadedFile['filename'], path=uploadedFile['path'])
            try:
...
class markov(Resource):
    ...
    def get(self, id):
        ...
        HcStat = HsHcstat.query.filter(HsHcstat.id == id).first()
    ...
    def delete(self, id):
        ...
        Deletes HcStat file.
        """
        markov = HsHcstat.query.filter(HsHcstat.id == id).one()
        markov.deleted = True
        db.session.commit()
        return {
            'status': True,
...
class markovMakeFromDictionary(Resource):
    ...
    def post(self):
        ...
        hcstats = HsHcstat(name=filename + '.hcstat2', path=path)
        try:
            db.session.add(hcstats)
            db.session.commit()
        except exc.IntegrityError as e:
