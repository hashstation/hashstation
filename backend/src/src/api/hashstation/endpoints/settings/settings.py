...
from src.api.hashstation.endpoints.settings.argumentsParser import settings_arguments
from src.api.hashstation.endpoints.settings.responseModels import settings_model
from src.api.hashstation.responseModels import simpleResponse
from src.database import db
from src.database.models import HsSettings, HsNotification
...
class settings(Resource):
    ...
    def get(self):
        ...
        """
        settings = HsSettings.query.filter_by(user_id=current_user.id).one_or_none()
        return settings
    ...
    def post(self):
        ...
        if discord_notifications:
            ...
            if not settings.discord_notifications: # If discord notifications were previously disabled, mark all notifications as sent
                HsNotification.query.filter(HsNotification.user_id == current_user.id).update({HsNotification.discord_sent: True})
        ...
        if telegram_notifications:
            ...
            if not settings.telegram_notifications:
                HsNotification.query.filter(HsNotification.user_id == current_user.id).update({HsNotification.telegram_sent: True})
        ...
        if email_notifications:
            ...
            if not settings.email_notifications:
                HsNotification.query.filter(HsNotification.user_id == current_user.id).update({HsNotification.email_sent: True})
