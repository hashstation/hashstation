...
from src.api.hashstation.endpoints.pcfg.argumentsParser import pcfg_parser, pcfgFromFile_parser, \
makePcfgFromDictionary_parser
from src.api.hashstation.endpoints.pcfg.functions import readingFromFolderPostProcces, \
...
from src.api.hashstation.endpoints.pcfg.responseModels import pcfgs_model, pcfgData_model, \
    pcfg_model, pcfgTree_model
from src.api.hashstation.functions import shellExec, fileUpload, allowed_file, getFilesFromFolder, directory_tree
from src.api.hashstation.responseModels import simpleResponse, file_content
from src.api.hashstation.argumentsParser import path
from src.database import db
from src.database.models import HsPcfg, HsDictionary
...
class pcfgCollection(Resource):
    ...
    def get(self):
        ...
        """
        return {'items': HsPcfg.query.filter(HsPcfg.deleted == False).all()}
...
class pcfg(Resource):
    ...
    def get(self, id):
        ...
        pcfg = HsPcfg.query.filter(HsPcfg.id == id).first()
        if not pcfg:
    ...
    def delete(self, id):
        ...
        Deletes pcfg
        """
        pcfg = HsPcfg.query.filter(HsPcfg.id == id).one()
        pcfg.deleted = True
        db.session.commit()
...
class pcfgMakeFromDictionary(Resource):
    ...
    def post(self):
        ...
        dict = HsDictionary.query.filter(HsDictionary.id == args['dictionary_id']).first()
        if not dict:
