...
from src.api.hashstation.endpoints.bins.responseModels import bin_model, bin_with_jobs_model, bins_model
from src.api.hashstation.endpoints.bins.argumentsParser import name, position, job_assignment
from src.api.hashstation.responseModels import simpleResponse

from src.database import db
from src.database.models import HsBin, HsJob
...
class bins(Resource):
    ...
    def get(self):
        ...
        """
        return {'items': HsBin.query.order_by(HsBin.position).all()}
...
class assign_bin(Resource):
    ...
    def put(self, id):
        ...
        # include if any
        jobs = HsJob.query.filter(HsJob.id.in_(include)).all()
