...
from src.api.hashstation.attacks.functions import check_mask_syntax, compute_keyspace_from_mask, compute_prince_keyspace, compute_rules_keyspace
from src.api.hashstation.functions import shellExec, lenStr
from src.database import db
from src.database.models import HsMask, HsDictionary, HsRule, HsHcstat, HsCharset, HsJobDictionary, HsJobRule
...
def process_job_0(job):
    ...
    for dictObj in job['attack_settings']['left_dictionaries']:
        dict = HsDictionary.query.filter(HsDictionary.id == dictObj['id']).first()
        if not dict:
...
def post_process_job_0(data, db_job):
    for dict in data['attack_settings']['left_dictionaries']:
        jobDict = HsJobDictionary(job_id=db_job.id, dictionary_id=dict['id'])
        db.session.add(jobDict)

    for rule in data['attack_settings']['rules']:
        jobRule = HsJobRule(job_id=db_job.id, rule_id=rule['id'])
...
def post_process_job_1(data, db_job):
    ...
    for dict in data['attack_settings']['right_dictionaries']:
        jobDict = HsJobDictionary(job_id=db_job.id, dictionary_id=dict['id'], is_left=False)
        db.session.add(jobDict)
...
def process_job_3(job, hashcatKeyspace=True):
    ...
    if job['attack_settings'].get('markov'):
        markov = HsHcstat.query.filter(HsHcstat.id == job['attack_settings']['markov']['id']).first()
        if not markov:
...
def post_process_job_3(data, db_job):
    for mask in data['mask_table']:
        db_mask = HsMask(job_id=db_job.id,
                         mask=mask['mask'],
                         current_index='0',
