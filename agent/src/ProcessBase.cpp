...
void ProcessBase::setStartTime() {
  start_time_ = AgentUtils::timeInMs();

  Logging::debugPrint(Logging::Detail::ObjectManipulation, "Process set start time : " + AgentUtils::toString(start_time_));
}

void ProcessBase::setStopTime() {
  stop_time_ = AgentUtils::timeInMs();

  Logging::debugPrint(Logging::Detail::ObjectManipulation, "Process set stop time : " + AgentUtils::toString(stop_time_));
}
...
double ProcessBase::getExecutionTime() const {
  ...
  unsigned long long end_time = (stop_time_ > 0 ? stop_time_ : AgentUtils::timeInMs());
  ...
  Logging::debugPrint(Logging::Detail::GeneralInfo, "Process execution time : " + AgentUtils::toString(elapsed_time_seconds));
...
std::string ProcessBase::readErrPipeLine() {
  ...
  AgentUtils::runtimeException("err_pipe_ is nullptr. Can't read.");
  return "";
}
...
std::string ProcessBase::readOutPipeLine() {
  ...
  AgentUtils::runtimeException("out_pipe_ is nullptr. Can't read.");
  return "";
}
