...
void PipeWindows::createPipe(bool is_NONBLOCK_) {
  ...
  AgentUtils::runtimeException("CreatePipe() failed", GetLastError());

  if (is_NONBLOCK_) {
    DWORD mode = PIPE_NOWAIT;
    if (!SetNamedPipeHandleState(read_, &mode, NULL, NULL)) {
       AgentUtils::runtimeException("SetNamedPipeHandleState() failed", GetLastError());
    }
  }
}
...
int PipeWindows::readChar(char& c) {
  ...
  if (PeekNamedPipe(read_, NULL, read_size, NULL, &n_read_chars, NULL) == 0) {
    if ((error_ = GetLastError()) == ERROR_BROKEN_PIPE) {
      ...
      AgentUtils::runtimeException("PeekPipe() failed", GetLastError());
    }
  }

  Logging::debugPrint(Logging::Detail::DevelDebug, POSITION_IN_CODE + " Characters to read: " + AgentUtils::toString(n_read_chars));

  if (n_read_chars >= read_size) {
    //call the blocking version since we know a char is ready
    c = readChar();
    Logging::debugPrint(Logging::Detail::DevelDebug, POSITION_IN_CODE + " Characters read: " + AgentUtils::toString(n_read_chars));
    return n_read_chars;
  } else {
    return -1;
  }
...
int PipeWindows::readChar()
{
  ...
  if (ReadFile(read_, &c, sizeof(c), &n_read_chars, NULL) == 0) {
    if ((error_ = GetLastError()) == ERROR_BROKEN_PIPE) {
      ...
      AgentUtils::runtimeException("ReadFile() failed", GetLastError());
    }
  }
  return c;
}
...
int PipeWindows::writeMessage(std::string& message) {
  ...
  if (!WriteFile(write_, message.data(), message.length(), &written_chars, NULL)) {
    ...
    {
      AgentUtils::runtimeException("WriteFile() failed", GetLastError());
    }
  }
