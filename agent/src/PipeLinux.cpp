...
void PipeLinux::redirectPipeEnd(int pipe_end, int target) {
  if (dup2(pipe_end, target) < 0) {
    AgentUtils::runtimeException("dup2 failed with target", errno);
  }
}
...
void PipeLinux::waitForAvailableOutput() const
{
  ...
  do
  {
    ...
    {
      AgentUtils::runtimeException("poll failed unexpectedly", errno);
    }
...
int PipeLinux::readChar(char& c) {
  ...
  switch (n_read_chars) {
    ...
    if (errno == EAGAIN) {
      ...
      AgentUtils::runtimeException("read() failed", errno);
      return 0;
    }
    case 0:
    closeRead();
...
int PipeLinux::readChar()
{
  ...
  if((res = readChar(c)) != 1)
  {
    ...
    AgentUtils::runtimeException("readChar failed even when pipe should be readable", errno);
  }
  return c;
}
...
int PipeLinux::writeMessage(std::string& message) {
  ...
  if (result == -1) {
    if (errno == EPIPE) {
      ...
      AgentUtils::runtimeException("writeMessage: broken pipe", errno);
    }
  }
  return result;
}
