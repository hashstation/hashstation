...
void FilesysBase::simplifyPath(std::string& path) {
  ...
  if ((found = path.find(current_directory)) != std::string::npos) {
    Logging::debugPrint(Logging::Detail::DevelDebug, path + " found " + current_directory + " at index " + AgentUtils::toString(found));
    Logging::debugPrint(Logging::Detail::DevelDebug, "Current directory size " + AgentUtils::toString(current_directory.size()) + " replacing till " +AgentUtils::toString(current_directory.size()-1));
...
void FilesysBase::relativeToAbsolute(const std::string& relative_path, std::string& absolute_path) {
  ...
  if (relative_path.size() > 0 && relative_path[0] == '.') {
    ...
    if (getcwd(path_buffer, PATH_MAX) != path_buffer) {
      ...
      AgentUtils::runtimeException("getcwd() failed: ", errno);
    }
