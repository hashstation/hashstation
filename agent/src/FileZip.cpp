...
void FileZip::decodeEntry(size_t entry_number, zip_stat_t *entry_stats) {
  ...
  unsigned long long start = AgentUtils::timeInMs();

  if (!(entry = zip_fopen_index(handle_, entry_number, ZIP_FL_ENC_GUESS))) {
    AgentUtils::runtimeException("zip_fopen_index() failed during extraction on entry: " + entry_name, errno);
  }
  ...
  if ((read_bytes = zip_fread(entry, data_buffer.data(), entry_stats->size)) < 0) {
    AgentUtils::runtimeException("zip_fread() failed during extraction on entry: " + entry_name, errno);
  }

  Logging::debugPrint(Logging::Detail::DevelDebug,
    "compressed size : " + AgentUtils::toString(entry_stats->comp_size) +
    " real size : " + AgentUtils::toString(entry_stats->size) +
    " buffer size : " + AgentUtils::toString(data_buffer.size()) +
    " read size : " + AgentUtils::toString(read_bytes)
  );
  ...
  Logging::debugPrint(Logging::Detail::DevelDebug, " entry decoding lasted: " + AgentUtils::toString(AgentUtils::timeInMs() - start) + "ms");

  if (zip_fclose(entry) < 0) {
    AgentUtils::runtimeException("zip_fclose() failed during extraction on entry: " + std::string(entry_name), errno);
  }
}
...
void FileZip::extractEntry(size_t entry_number) {
  ...
  Logging::debugPrint(Logging::Detail::DevelDebug, " entry extraction lasted: " + AgentUtils::toString(AgentUtils::timeInMs() - start) + "ms");
}
...
void FileZip::close() {
  if (zip_close(handle_) < 0) {
    AgentUtils::runtimeException("zip_close() failed on archive:" + getRelativePath(), errno);
  }
...
void FileZip::extract() {
  ...
  Logging::debugPrint(Logging::Detail::GeneralInfo, "FileZip: extracted " + AgentUtils::toString(entries) + " entries from archive " + getRelativePath());
...
void FileZip::open() {
  if ((handle_ = zip_open(getRelativePath().c_str(), 0, &error_)) == NULL) {
    AgentUtils::runtimeException("zip_open() failed on archive:" + getRelativePath() + " with zip_error code " + AgentUtils::toString(error_), errno);
  }
