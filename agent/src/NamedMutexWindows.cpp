...
public:
		...
		handle(CreateMutexA(nullptr, false, name.c_str()))
	{
		if(!handle)
		{
			AgentUtils::runtimeException("Failed to create a named mutex", GetLastError());
		}
	}
	~NamedMutexHandle()
	{
...
void NamedMutex::lock()
{
	...
	{
		AgentUtils::runtimeException("Failed to wait for named mutex", GetLastError());
	}
	m_locked = true;
...
void NamedMutex::unlock()
{
	...
	{
		AgentUtils::runtimeException("Failed to release named mutex", GetLastError());
	}
	m_locked = false;
