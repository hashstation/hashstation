...
void ConfigTask::convertLineToOption(std::string& line) {
  ...
  return;
    ...
    if (verifyFields(fields)) {
      ...
      AgentUtils::runtimeException("ConfigTask: value lengths doesn't match or option isn't supported");
    }
  }
}
...
void ConfigTask::parseLine(const std::string& line, std::vector<std::string>& fields) {
  ...
  line.substr(line.length() - fat_delimeter.length(), fat_delimeter.length()) != fat_delimeter) {
    AgentUtils::runtimeException("Invalid config syntax in file: " + path_);
  }
...
bool ConfigTask::verifyFields(std::vector<std::string>& fields) {
  ...
  if (isSupported(fields[0])) {
    ...
    Logging::debugPrint(Logging::Detail::CustomOutput, "ConfigTask verifying field : '" + fields[0] + "' value : '" + fields[3] + "' length: sent size = " + fields[2] + ", actual string size = " + AgentUtils::toString(fields[3].length()));

    if (fields[3].length() == AgentUtils::stoul(fields[2], nullptr, 10))
    return true;
  }
  return false;
}
...
void ConfigTask::print() {
  ...
  Logging::debugPrint(Logging::Detail::ObjectContentRevision, "ConfigTask file contained " + AgentUtils::toString(options_.size()) + " options.");
}
