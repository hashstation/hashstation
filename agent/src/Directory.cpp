...
void Directory::init() {
  if (!relative_path_.empty() && !isDirectory(relative_path_))
    AgentUtils::runtimeException("stat() failed on " + relative_path_, errno);
  FilesysBase::init();
}
...
void Directory::scanForEntities() {
  ...
  if ((directory = opendir (absolute_path_.c_str())) != NULL) {
    ...
    AgentUtils::runtimeException("opendir() failed", errno);
  }
}
...
switch (errno)
{
  ...
  /** parent doesn't exist, try to create it */
  {
    ...
    AgentUtils::runtimeException("mkdir() failed ", errno);
...
void Directory::printDirectories() {
  ...
  Logging::debugPrint(Logging::Detail::ObjectContentRevision, "Directory contains " + AgentUtils::toString(directories_.size()) + " directories.") ;
}
...
void Directory::printFiles() {
  ...
  Logging::debugPrint(Logging::Detail::ObjectContentRevision, "Directory contains " + AgentUtils::toString(files_.size()) + " files.");
}
