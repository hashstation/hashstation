...
void File::init() {
  ...
  if (!isFile(relative_path_)) {
    AgentUtils::runtimeException(relative_path_ + " is not file");
  }
...
int File::openStream(std::fstream &file, std::fstream::openmode mode, const std::string &file_name) {
  ...
  if (!file.is_open()) {
    AgentUtils::runtimeException("open() failed", errno);
  }
...
void File::setExecutableMode() {
  ...
  if (chmod(getRelativePath().c_str(), 0755) == -1) {
    AgentUtils::runtimeException("chmod() failed for file: " + name_);
  }
#endif
}
