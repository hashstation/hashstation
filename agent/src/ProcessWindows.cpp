...
void ProcessWindows::launchSubprocess() {
  ...
  * stderr.txt */
  ...
  if (!SetHandleInformation(startup_info_.hStdError, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT)) {
    AgentUtils::runtimeException("SetHandleInformation() for stderr failed", GetLastError());
  }

  if (!SetHandleInformation(startup_info_.hStdOutput, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT)) {
    AgentUtils::runtimeException("SetHandleInformation() for stdout failed", GetLastError());
  }
  ...
  if (startup_info_.hStdInput != GetStdHandle(STD_INPUT_HANDLE) && !SetHandleInformation(startup_info_.hStdInput, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT)) {
    AgentUtils::runtimeException("SetHandleInformation() for stdin failed", GetLastError());
  }
  ...
  &process_information_)           // Pointer to PROCESS_INFORMATION structure
    ...
    err_pipe_->closeAll();
    out_pipe_->closeAll();
    AgentUtils::runtimeException("CreateProcess() failed", GetLastError());
  }
...
int ProcessWindows::getExitCode() const {
  ...
  if(GetExitCodeProcess(process_information_.hProcess, &exit_code) == 0) {
    AgentUtils::runtimeException("GetExitCodeProcess() failed", GetLastError());
  }

  Logging::debugPrint(Logging::Detail::DevelDebug, "ProcessWindows : got exit_code : " + AgentUtils::toString(exit_code) +
  " which as int is : " + AgentUtils::toString((int)exit_code)
);
