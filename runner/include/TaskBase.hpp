...
protected:
  ConfigTask task_config_; /**< Representant of task config file received from
                              server */

  ConfigHost host_config_; /**< Representant of host based config file set by
                              host administrator */

  Directory directory_; /**< Representatn of directory where is program executed
                           from */
  ...
  std::string
      mode_; /**< Mode of the task a-benchmark_all, b-benchmark, n-normal */
  std::string output_file_; /**< Name of the hashcat output file */
  std::string
      workunit_name_; /**< Name of the workunit assigned by boinc server */
...
public:
   ...
   */
  TaskBase(Directory &directory, ConfigTask task_config,
  ...
  /**
   * @brief   Pure virtual function setting total_keyspace_ value base on
   * ConfigTask object
   */
